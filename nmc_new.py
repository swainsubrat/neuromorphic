...
def main():
    ...
    train_acc_log, test_acc_log, train_loss_log, test_loss_log = [], [], [], []
    ...
    for ep in range(1, args.epochs + 1):
        ...
        print(f"[{args.update.upper()}] Epoch {ep:03d} | Train Loss {train_loss:.4f} | Test Acc {test_acc:.2f}%")

        # Compute train accuracy once per epoch (after updates)
    ...
    # Plot: Epochs vs Train/Test Accuracy (patterned lines)
    ...
    plt.legend(frameon=False)
    plt.tight_layout()
    plt.savefig(os.path.join(outdir, "train_loss_vs_epoch.png"), dpi=600)
