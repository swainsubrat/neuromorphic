...
def get_lr(epoch, total_epochs, lr_max, lr_min, warmup_epochs=5):
    """
    Linear warmup for first `warmup_epochs`, then cosine decay.
    Warmup prevents large early gradients from pushing weights to saturation.
    """
...
def main():
    ...
    for ep in range(1, args.epochs + 1):
        lr = get_lr(ep - 1, args.epochs, args.lr_max, args.lr_min, args.warmup)
        lr_log.append(lr)
